...
def listar_creditos(conn: Connection, pkcliente: int) -> list[dict]:
    """Créditos del cliente usando el periodo más reciente de cada crédito."""
    sql = text(
        """
        ...
        JOIN fagcuentacredito fa
          ON fa.pkcuentacredito = cr.pkcuentacredito
         AND fa.periodomes = (
               SELECT MAX(fa2.periodomes) FROM fagcuentacredito fa2
               WHERE fa2.pkcuentacredito = cr.pkcuentacredito)
        ...
        ORDER BY fa.fechadesembolsocredito DESC, cr.pkcuentacredito DESC
        """
    )
    return [
        dict(r)
        for r in conn.execute(sql, {"pk": pkcliente}).mappings().all()
    ]


def buscar_cuenta_credito(conn: Connection, codcuentacredito: str) -> dict | None:
    """Datos base de un crédito + agencia/moneda/producto/asesor del periodo más reciente."""
    sql = text(
        """
    ...
    row = conn.execute(sql, {"cod": codcuentacredito}).mappings().first()
    return dict(row) if row else None
